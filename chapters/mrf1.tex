...
Each robot periodically broadcasts a message containing:
  ...
  \item a \textbf{matching} of the neighbors of the sender robot to out-going edges of its
...
relationship, called \textbf{matching} between its neighbors and the out-going edges from its role vertex. 
\begin{defn}
\label{def:matching}
  Given a robot $r_i$ and a role vertex $v$ for that robot, let the lattice
  graph edge set
    $O=\{\emptyset, e_{v}^w, e_{v}^u, \ldots\}$
...
Given an edge $e$, we can use its transformation $T(e)$ to find the desired position.
\begin{equation}
  \label{eq:trans-pos}
  \left[\bar{x}^{(i)} \quad \bar{y}^{(i)} \quad 1\right]^\top = T(e) \left[0 \quad 0 \quad 1\right]^\top
\end{equation}
