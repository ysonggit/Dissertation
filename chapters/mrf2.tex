...
The existing lattice formation may be destroyed and the cluster of robots would re-organize themselves by constructing a new authority tree.
...
in terms of Algorithm~\ref{alg:vacancy}, which describes the procedure of computing a vacancy for a stable robot. 
...
can not break their connectivity.  
%
...
\begin{proof}
    ...
    are the maximum $n-1$ robots to be relocated. 
    %
...
If a new robot, whose ID is higher than any other robots, joins in the network, then the robots can not re-organize themselves because there is no way for those stable robots to reset their status back to unstable.
\end{enumerate}
