...
The rest of this chapter are organized as following: in Section~\ref{sec:related-cga}, 
...
against various range spaces, we draw conclusions of our approach in the last section.
...
robots navigation, simultaneous localization and mapping (SLAM), and searching problems, etc.,
...
the landmarks' locations and robot's position are estimated by a likelihood
maximized under probabilistic constraints~\cite{ThrBurFox98}. In their work, the
probabilistic models are used to represent and reason about both of robot's
sensing and motion. Bayesian estimation algorithms are also used in Thrun's
...
filter~\cite{Kal60} was adopted to represent and track a number of pose
...
approach, for example, following algorithms solve the problems in spite of limitations
...
(GNT) to solve different visibility-based tasks with minimal sensing requirements in unknown planar environments, whose boundaries are piecewise smooth closed curves, with finite number of non-smooth points~\cite{TovGuiLav04}. 
The GNT structure reflects the path information in the information space, it is constructed from the online sensor measurements and updated during the robot's moving. 
%
...
determine a path that passes a detector, whose range extends beyond the robot's
...
our work contributed to demonstrate that such precise I-states are not always necessary for a robot to complete its task, additionally, the reasonable constrained approximations can be computed much
more efficiently~\cite{SonOka12}.
...
%
The detail of the robot model is described as following:
\begin{enumerate}
\item \emph{State space} $X = \Real^2$: a discrete planar state space $X$ is
...
\item \emph{State transition function} $F : X \times U \to \pow(X)$: a
  ...
  action. The angular noise is bounded by a fixed angle $\delta\sub{ang}$, and
  the translation noise is bounded by $\delta\sub{trans}||u||$. Thus, the
...
%
Generally, an information space is essentially a special state space in which the history of action and sensor observation are involved. 
%
Here we follow the convention from LaValle's book~\cite{Lav06}. 
%
Assuming at every stage a sensor observation is obtained and an action is made by the robot, the
sensor history at stage $k$ is
\begin{equation}
...
was applied to the time when the latest action $u_{k-1}$ is applied. Thus, we have
\begin{equation}
...
\end{defn}
%
Combining the sensor-action history with the initial condition, the history
I-state can be obtained, denote the history I-state at stage $k$ using $\eta_k$, we have
\begin{equation}
...
computed. Recall Equation~\ref{eq:hist-istate2}, we have $\eta_{k+1} =
(\eta_{k}, {u}_{k}, y_{k})$, such that
\begin{equation}
  X_{k+1}(\eta_{k+1}) = X_{k+1}(\eta_{k}, {u}_{k}, y_{k}).
\end{equation}
%
Then two set operations are applied to compute the state subset
$X_{k+1}(\eta_{k+1})$: 
\begin{enumerate}
...
complete the task, so that the plan $\pi: \Ispace \to U$, $u_k =
\pi(\eta_k)$.
...
remain always a member of this range space, so that $A_k \in \Rspace$.
\begin{defn}
  \label{def:rspace}
...
Recall in a 2D plane, an information state can be represented using a compact set, thus, for any compact set $S \subset \Real^2$, let $\sed(S)$ denote the smallest disk enclosing $S$.
%
Then we define
\begin{flalign}
  \label{eq:tdisk} 
...
\end{flalign}
%
Recall that we mentioned in the section~\ref{sec:robot-mod}, the action and
noise models we use in this problem are addictive.
%
...
%
Computing Minkowski sums of disks consists of the addition of their centers and radius, so it takes constant time to compute $T\disk$.  
%
Algorithms are known to evaluate $O\disk$ in constant time when the observation preimages are quarter-planes~\cite{OKa08b} or circles~\cite{OKaXu12}.
  
Since both $A_k$ and $\sed(\Theta(u_k))$ are disks, the result is still in $\Rdisk$.  
%
Similarly, we define
\begin{equation} 
...
\item Step 1: find the minimal axis-aligned bounding box for the bounded noise set
  ...
  Since the set $\Theta(u_k)$ is represented by a list of vertices on its boundary, this
  operation takes linear time to find the extremal coordinates in each direction
  from this vertex list.
...
%
In our experiments, every $H(y_k)$ is assumed to be a planar disk.  
...
\item Step 2: for each of four axis-aligned extremal points of the disk
  ...
  points, check if it is contained in $A_k$.
...
locations of $l_1,\ldots,l_m$ which can be sensed by the robot. 
%
...
%
Moreover, to navigate the robot, we place a sequence of way points $w_1,\ldots,w_n$
...
sequentially visiting each of the way points, by utilizing the imperfect sensed
information and its noise movements. 
%
We consider the task is completed successfully if and only if the robots finishes visiting the way points in finite time. 
%
Otherwise, we consider the task is failed if the robot collides with
an obstacle or it takes too long time for the robot to reach the final way point.
...
have conducted experiments using three distinct environments
(Figure~\ref{fig:env}), and three distinct range spaces: $\Rspace_{disk}$,
$\Rspace_{rect}$, and $\Rspace_{dblrect}$.  
%
The experiments are implemented in C++. All the simulations run on a GNU/Linux PC with Intel Core i7 CPU, 2.8GHz and 8GB memory.

We present experiments that measure these approximation ratios, along with the robot's success rate in completing the navigation task using each of these range spaces.
\begin{figure}
  \centering
...
In each environment, we have performed two series of experiments. 
%
First, we vary the number of landmarks $N$ between $5$ and $250$ in increments of $5$ and calculated the
...
%
Figure~\ref{fig:sucRate1}, \ref{fig:sucRate2}, and \ref{fig:sucRate3} show the average success rate for three environments, respectively. 
%
We conclude that similar performance to the true I-state can be achieved using the approximated I-states, especially when $\Rrect$ and $\Rdrect$ are used.
...
we conclude that using the double-rectangle approximated I-states yields better
performance over using the other types of range spaces.


Moreover, experiments are conducted to evaluate the impact of the selection of axes. 
%
Considering that both $\Rrect$ and $\Rdrect$ are constructed based on only
axis-aligned rectangles, these experiments are practical and important to give a
comprehensive evaluation of the performance of CGA method. 
%
We rotate each environment for a degree between $0$ and $2\pi$ radians, in increments of $\pi/12$, 
and run $10$ trials with $250$ landmarks for each environment.
%
...
%
The results for the obstacle-free environment are omitted since the robot always accomplish the
navigation task in every trial. 
%
The results demonstrate that there is no substantial degradation of CGA's performance as a function of environment rotation.
...
We also have conducted experiments to compare the computation time of our approximated
I-states with the corresponding computation time of the exact I-states.  
%
We use $N=300$ landmarks and run $10$ trials for each combination of the range space
and the environment.  
%
At the same time, we compute the approximation ratio $Q$ as defined in Equation~\ref{eq:error}.  
%
The results are collected in Table~\ref{tab:exp-data}. 
%
...
%
Moreover, the statistics demonstrate that the double-rectangle range space provides the highest approximation quality.
...
These experiments have verified the effectiveness of our over-approximating approach.
%
Specifically, the innovation of the range space as the union of
two rectangles improves the approximation accuracy. 
%
We conclude that our method of representing a robot's uncertainty and reasoning about its state is useful for robot to complete specific tasks, such as navigation task based on the landmarks, even subject to a low approximation ratio.

However, it remains as future work to consider additional range spaces.  
%
Particularly, this approach can be generalized to $k$-fold unions of rectangles, without any substantial changes to the algorithm.  
...
Moreover, the range spaces that allow more general polygonal shapes are worthy to be considered. 
The challenge is how to maintain efficiency by limiting their number of vertices, and by applying appropriate expansion operations to enforce this limit.
